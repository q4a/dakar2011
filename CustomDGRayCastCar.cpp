	...
	:NewtonCustomJoint(2 * maxTireCount, carBody, NULL)
{
	...
	m_currentChassisRotation = 1.0;
	m_chassisRotationRate = 0.005f;
...
void CustomDGRayCastCar::SetVarChassisRotationRate (dFloat value)
{
	m_chassisRotationRate = value;
}
...
dFloat CustomDGRayCastCar::GenerateTiresSteerAngle (dFloat value)
{
    ...
    dFloat maxSteerAngle = m_maxSteerAngle * value;
    if (m_steerAngle < maxSteerAngle)
    {
   		m_steerAngle += m_maxSteerRate;
   		/*
		if (dAbs(maxSteerAngle) < EPSILON && m_steerAngle > -m_maxSteerRate*1.0f )
		{
        ...
        */
		...
		if (dAbs(maxSteerAngle) < EPSILON && m_steerAngle > -m_maxSteerRate*1.0f )
		{
    		m_steerAngle = 3.0f*m_maxSteerRate;
        }
        else
        {
        */
...
void CustomDGRayCastCar::ApplyChassisTorque (const dVector& vForce, const dVector& vPoint)
{
	...
	// here: add Scale to vForve
	Torque = ( vPoint - M.TransformVector( dVector( com.m_x, com.m_y, com.m_z, 1.0f ) ) ) * vForce/*.Scale(m_chassisRotationLimit)*/ ;
...
//		tire.m_lateralDir = tire.m_lateralPin; 
		...
		if ( tire.m_posit < tire.m_suspensionLenght )  {
			...
			// here
			tire.m_TireForceTorque = tireForce/*.Scale(m_chassisRotationLimit)*/;
			/*
			...
			*/
			if ( tire.m_tireIsOnAir == 0 ) {
                //printf("%f\n", tire.m_suspensionMatrix.m_right.Scale( tire.m_torque ).m_y);
                // here
				tire.m_TireForceTorque += tire.m_suspensionMatrix.m_right.Scale( tire.m_torque /* m_chassisRotationLimit*/); 
				if ( dAbs( m_curSpeed ) != 0.0f ) {
                    // here
					tire.m_TireForceTorque += tire.m_suspensionMatrix.m_front.Scale( tire.m_turnforce /* m_chassisRotationLimit*/); 
				}
                ...
                // here: add * m_chassisRotationLimit
				DisplaceAxelPosit += tire.m_suspensionMatrix.m_front.Scale(tire.m_MovePointForceFront /* m_chassisRotationLimit*/);
				DisplaceAxelPosit += tire.m_suspensionMatrix.m_right.Scale(tire.m_MovePointForceRight /* m_chassisRotationLimit*/);
				DisplaceAxelPosit += tire.m_suspensionMatrix.m_up.Scale(tire.m_MovePointForceUp /* m_chassisRotationLimit*/);
				//
	...
	{
	   m_omegaCD = 10;
	   m_currentChassisRotation = m_chassisRotationLimit;
    }

    m_vehicleOnAir = new_vehicleOnAir;

    if (!m_vehicleOnAir && m_currentChassisRotation < 1.0f/*&& m_omegaCD > 0*/)
    {
        //printf("apply omega correction\n");
        //m_omegaCD--;
        //m_currentChassisRotation += m_chassisRotationRate;
