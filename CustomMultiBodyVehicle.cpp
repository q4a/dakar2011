...
CustomMultiBodyVehicleTire::CustomMultiBodyVehicleTire(
	const NewtonBody* hubBody, 
	NewtonBody* tire, 
	dFloat suspensionLength, dFloat springConst, dFloat damperConst, dFloat radio, float mass)
	:NewtonCustomJoint(6, hubBody, tire), m_tireIsOnAir(false), m_mass(mass), tireBody(tire)
{
	dFloat dmass;
	dFloat Ixx;
	dFloat Iyy;
	...
	NewtonBodyGetMassMatrix(tire, &dmass, &Ixx, &Iyy, &Izz);
	m_Ixx = Ixx;
...
//		m_effectiveSpringMass = mass0 * mass1 / (mass0 + mass1);
	NewtonBodyGetMassMatrix(hubBody, &dmass, &Ixx, &Iyy, &Izz);
	m_effectiveSpringMass = dmass * 0.25f;
...
CustomMultiBodyVehicleTire::~CustomMultiBodyVehicleTire(void)
{
}

void CustomMultiBodyVehicleTire::GetInfo (NewtonJointRecord* info) const
{
}

dFloat CustomMultiBodyVehicleTire::GetSteerAngle () const
{
	return m_steeAngle;
}

void CustomMultiBodyVehicleTire::SetSteerAngle (dFloat angle)
{
...
void CustomMultiBodyVehicleTire::SetTorque (dFloat torque)
{
	m_enginetorque = torque;
}

void CustomMultiBodyVehicleTire::SetBrakeTorque (dFloat torque)
{
	m_brakeToque = torque;
}

void CustomMultiBodyVehicleTire::SetAngulaRollingDrag (dFloat angularDampingCoef)
{
	m_angularDragCoef = angularDampingCoef;		
}


void CustomMultiBodyVehicleTire::ProjectTireMatrix()
{
...
void CustomMultiBodyVehicleTire::SubmitConstraints (dFloat timestep, int threadIndex)
{
...
NewtonBody* CustomMultiBodyVehicle::GetTireBody(int tireIndex) const
{
	return m_tires[tireIndex]->tireBody;//GetBody1();
}
...
//	NewtonBodySetMaterialGroupID (tire, woodID);
	...
	// add a single tire
	m_tires[m_tiresCount] = new CustomMultiBodyVehicleTire (GetBody0(), tire, suspensionLength, springConst, springDamper, radius, mass);
	m_tiresCount ++;
